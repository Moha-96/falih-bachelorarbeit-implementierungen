package de.hhu.rich.warenkorb.infrastructure.presentation.api;

import de.hhu.rich.warenkorb.application.ApplicationService;
import de.hhu.rich.warenkorb.infrastructure.presentation.warenkorb.WarenkorbModel;
import de.hhu.rich.warenkorb.infrastructure.presentation.warenkorb.WarenkorbModelMapper;
...
public class WarenkorbController {
    ...
    @GetMapping("/")
    public ResponseEntity<WarenkorbModel> zeigeWarenkorb(@ModelAttribute("warenkorbID") String warenkorbID) {
        WarenkorbModel warenkorb = WarenkorbModelMapper.INSTANCE.vonWarenkorbZuModel(applicationService.getWarenkorb(UUID.fromString(warenkorbID)));

        return new ResponseEntity<>(warenkorb, HttpStatus.OK);
    }

    @PostMapping("/fuegeHinzuArtikel/{artikelID}/{anzahl}")
    public ResponseEntity<String> legeArtikelInWarenkorb(@ModelAttribute("warenkorbID") String warenkorbID,
                                                             @PathVariable String artikelID,
                                                             @PathVariable int anzahl) {
        try {
            UUID warenkorb = UUID.fromString(warenkorbID);
            UUID artikel = UUID.fromString(artikelID);
            applicationService.fuegeArtikelZumWarenkorbHinzu(artikel, anzahl, warenkorb);
    ...
    public ResponseEntity<String> entferneArtikelAusWarenkorb(@ModelAttribute("warenkorbID") String warenkorbID,
                                                                 @PathVariable String artikelID) {
        try {
            UUID warenkorb = UUID.fromString(warenkorbID);
            UUID artikel = UUID.fromString(artikelID);
            applicationService.entferneArtikelAusWarenkorb(artikel, warenkorb);
    ...
    public ResponseEntity<String> reduziereAnzahlVonArtikelImWarenkorb(@ModelAttribute("warenkorbID") String warenkorbID,
                                                                          @PathVariable String artikelID,
                                                                          @PathVariable int anzahl) {
        try {
            UUID warenkorb = UUID.fromString(warenkorbID);
            UUID artikel = UUID.fromString(artikelID);
            applicationService.reduziereAnzahlVonArtikelImWarenkorb(artikel, anzahl, warenkorb);
